...
func NewArrow(t time.Time) Arrow {
	return Arrow{t}
}

func UTC() Arrow {
	return NewArrow(time.Now().UTC())
}

func Unix(sec int64, nsec int64) Arrow {
	return NewArrow(time.Unix(sec, nsec))
}
...
func (a Arrow) InTimezone(timezone string) Arrow {
	if location, err := time.LoadLocation(timezone); err == nil {
		return NewArrow(a.In(location))
	}
	return a
}

func (a Arrow) AddDays(days int) Arrow {
	return NewArrow(a.AddDate(0, 0, days))
}
...
func (a Arrow) AtBeginningOfSecond() Arrow {
	return NewArrow(a.Truncate(Second))
}

func (a Arrow) AtBeginningOfMinute() Arrow {
	return NewArrow(a.Truncate(Minute))
}

func (a Arrow) AtBeginningOfHour() Arrow {
	return NewArrow(a.Truncate(Hour))
}
